...
\begin{figure*}[t]
  ...
  \caption{\sysname architecture. Policies attach to an unmodified NCCL
  Core through its plugin ABI (dashed) and enter only via the verifier
  and JIT, which serve load and hot-reload. The profiler writes latency
  into shared maps the tuner reads.}
