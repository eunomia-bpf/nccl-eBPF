...
AllReduce throughput by 5--27\% over NCCL's default algorithm
...
is predictable: approximately $80 + 30n_{\text{lookup}} +
...
${\sim}$1.3\us of fixed overhead (${\sim}$4\% of the 31\us NVLink
baseline). This overhead comes from the plugin framework (shared
...
improving throughput by 5--27\% over NCCL's default NVLS
...
\texttt{nvlink\_ring\_mid\_v2} policy (17~lines of C) selects
...
range (steady-state, reps~4--5 of 5), while matching default
performance outside this range. The \texttt{noop} policy (not
...
passes the verifier (it is memory-safe) but causes 87--93\%
...
flow into a shared eBPF map, and the tuner ramps to the optimal
channel count. This validates \sysname's composability model: two
independently deployed eBPF programs cooperate through shared typed
maps, a capability absent from NCCL's native plugin architecture.
...
NVLink AllReduce throughput by 5--27\% over NCCL's default, with
