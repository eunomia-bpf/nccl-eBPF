...
% ACM camera-ready: copyright notice must appear (do not suppress it),
% and page numbers must be off.
...
The analogy is not exact, however, and where it breaks is where the
work lies. A verifier discharges obligations about one program on one
execution, but several of NCCL's invariants are neither. NCCL requires
that all ranks of a communicator agree on the algorithm they select,
or the collective hangs---a property over a \emph{set} of executions
that no single-program verifier can express. Identifying which of a
host's invariants fall outside the verifier, and where each is
discharged instead, is the substance of hosting eBPF behind an
existing library ABI.
...
cross-plugin state sharing, enabling
...
\subsection{Where Invariants Are Discharged}
\label{sec:invariants}

A verifier proves properties of one program on one execution. Several
of the invariants NCCL depends on are neither, and each must therefore
be discharged somewhere else. Some are \emph{runtime-valued}: the cost
table's dimensions and a communicator's channel limit are known only
at call time, so they are enforced by the trusted host rather than
proved statically. One is \emph{semantic}: cells marked unavailable
must not be overwritten, or NCCL selects a combination it cannot
execute. And one is \emph{distributed}: the ranks of a communicator
must agree, or the collective hangs. The verifier's guarantees are
fixed; what they are worth on a given host is set by how the
remaining invariants are discharged---by a trusted shim, by the host's
own re-validation, or by restricting policies to a class that
satisfies them by construction. The tensions below follow from this
division of labor.
...
\begin{figure*}[t]
  ...
  \caption{\sysname architecture. The plugin host loads policies
  in-process, where policies attach to an unmodified
  NCCL through its plugin ABI (dashed) and enter via the
  verifier and JIT for load and hot-reload. The host, runtime
   are trusted, the policies are not. The profiler
  writes latency into shared maps the tuner reads.}
...
\begin{lstlisting}[language=C, caption={Profiler-to-tuner closed loop.
  ...
  Null checks (lines 7, 18) are enforced by the verifier.},
...
so our native baseline layer clamps the policy's request. Communicator
...
and across 400{,}000 continuous invocations we observe zero lost calls.
...
net plugin prototype wraps the built-in Socket backend, and deeper
...
\paragraph{Rank agreement as a load-time check.}
Of the invariants in \S\ref{sec:invariants}, the distributed one is
the most interesting because it is the one no verifier can express---and
yet it is statically checkable in a useful special case. Identical
bytecode guarantees identical output only when a policy is a
deterministic function of rank-invariant inputs. A load-time check on
input provenance---rejecting policies whose output depends on
rank-local telemetry---would therefore make agreement hold by
construction rather than, as for the policies evaluated here, by
accident. Such a rule is host-specific: kernel eBPF hooks are CPU-local
and carry no agreement obligation, which is why the problem has no
counterpart there. It also conflicts with closed-loop adaptation, which
is telemetry-fed by definition; admitting only telemetry that is itself
replicated across ranks would reconcile the two. We have not
implemented this check, and leave it open.

The eBPF-for-extensions pattern may generalize beyond NCCL.
More generally, eBPF is increasingly deployed as replicas across a
fleet, where identical bytecode is relied on to give identical
behavior; that reliance is sound only while policy inputs are
replica-invariant.
...
% ============================================================================
\begin{acks}
We thank \textbf{Verda} for helpful discussions and for providing the
