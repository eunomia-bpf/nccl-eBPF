...
only at runtime. Some are runtime-valued: the cost table's dimensions
...
One is semantic: cells marked unavailable must not be overwritten, or
NCCL selects a combination it cannot execute. And one is distributed:
all ranks must select the same algorithm, or the collective hangs.
