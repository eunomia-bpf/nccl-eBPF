...
eBPF does not face. Kernel hooks are local, e.g. each CPU handles its own
packets, its own syscalls, so a verifier that checks one execution at
a time is enough. Distributed systems like NCCL are different: all
ranks must select the same algorithm, or the collective hangs. A
verifier still sees one execution; it cannot check that replicas
agree. Our insight is simple: same code on same inputs yields same
outputs. \sysname{} closes the gap by checking that policies read only
inputs shared across replicas.
