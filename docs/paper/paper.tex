...
verifier still sees one execution, and it cannot check replica
consistency. Our insight is simple: same code on same inputs yields same
...
must be consistent, or the collective hangs. The verifier's guarantees are
fixed; what they are worth on a given host is set by how the
remaining invariants are discharged, whether by a trusted shim, by the host's
...
Naive closed-loop adaptation breaks this rule because a policy that reads
rank-local telemetry can choose different algorithms on different ranks,
deadlocking the collective. The fix is to analyze the policy's computation
graph and ensure it depends only on globally consistent inputs. At load
time, the host can track which inputs flow into the output decision. If
the graph depends only on rank-invariant inputs (message size, collective
type), consistency is guaranteed. If the graph depends on rank-local
telemetry, the host can either reject the policy or require that telemetry
be replicated before the policy runs.
